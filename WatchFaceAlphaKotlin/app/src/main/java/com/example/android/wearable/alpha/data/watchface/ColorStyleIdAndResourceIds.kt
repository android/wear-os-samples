...
enum class ColorStyleIdAndResourceIds(
    ...
    companion object {
        ...
        fun toOption(context: Context, id: String): ListUserStyleSetting.ListOption {
            return when (id) {
                AMBIENT.id -> {
                    ListUserStyleSetting.ListOption(
                        id = UserStyleSetting.Option.Id(AMBIENT.id),
                        displayName = context.getString(AMBIENT.nameResourceId),
                        icon = Icon.createWithResource(
                            context,
                            AMBIENT.iconResourceId
                        )
                    )
                }
                RED.id -> {
                    ListUserStyleSetting.ListOption(
                        id = UserStyleSetting.Option.Id(RED.id),
                        displayName = context.getString(RED.nameResourceId),
                        icon = Icon.createWithResource(
                            context,
                            RED.iconResourceId
                        )
                ...
                GREEN.id -> {
                    ListUserStyleSetting.ListOption(
                        id = UserStyleSetting.Option.Id(GREEN.id),
                        displayName = context.getString(GREEN.nameResourceId),
                        icon = Icon.createWithResource(
                            context,
                            GREEN.iconResourceId
                        )
                    )
                }
                BLUE.id -> {
                    ListUserStyleSetting.ListOption(
                        id = UserStyleSetting.Option.Id(BLUE.id),
                        displayName = context.getString(BLUE.nameResourceId),
                        icon = Icon.createWithResource(
                            context,
                            BLUE.iconResourceId
                        )
                ...
                WHITE.id -> {
                    ListUserStyleSetting.ListOption(
                        id = UserStyleSetting.Option.Id(WHITE.id),
                        displayName = context.getString(WHITE.nameResourceId),
                        icon = Icon.createWithResource(
                            context,
                            WHITE.iconResourceId
                        )
                    )
                }
                else -> {
